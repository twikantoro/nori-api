...
router.get('/getFcmToken', async function (req, res, next) {
  db.collection('pengguna').doc(req.query.id_pengguna).get().then(doc => {
    if (!doc.data()) {
      //user tidak ditemukan, dibuat sekaligus update token
      let data = {
        fcmToken: req.query.fcmToken
      }
      db.collection('pengguna').doc(req.query.id_pengguna).set(data).then(wr=>{
        res.send(req.query.fcmToken)
      })
    } else {
      //user ditemukan, cek apakah token nya sama?
      if(doc.data().fcmToken==req.query.fcmToken){
        //tokennya sama, return token
        res.send(doc.data().fcmToken)
      } else {
        //tokennya beda, buat token lalu return token
        let data = {
          fcmToken: req.query.fcmToken
        }
        db.collection('pengguna').doc(req.query.id_pengguna).update(data).then(wr=>{
          res.send(req.query.fcmToken)
        })
      }
    }
